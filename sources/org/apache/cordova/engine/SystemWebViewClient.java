...
public class SystemWebViewClient extends WebViewClient {
    ...
    public void onReceivedSslError(WebView view, SslErrorHandler handler, SslError error) {
        try {
            if ((this.parentEngine.f10cordova.getActivity().getPackageManager().getApplicationInfo(this.parentEngine.f10cordova.getActivity().getPackageName(), 128).flags & 2) != 0) {
                handler.proceed();
            } else {
    ...
    public WebResourceResponse shouldInterceptRequest(WebView view, String url) {
        try {
            if (!this.parentEngine.pluginManager.shouldAllowRequest(url)) {
                ...
                return new WebResourceResponse(WebRequest.CONTENT_TYPE_PLAIN_TEXT, WebRequest.CHARSET_UTF_8, (InputStream) null);
            }
            ...
            return new WebResourceResponse(result.mimeType, WebRequest.CHARSET_UTF_8, result.inputStream);
        } catch (IOException e) {
