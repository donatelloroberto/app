...
class YuvConverter {
    ...
    public void convert(ByteBuffer buf, int width, int height, int stride, int srcTextureId, float[] transformMatrix) {
        ...
        if (this.released) {
            ...
            GLES20.glUniform2f(this.xUnitLoc, transformMatrix2[0] / ((float) width), transformMatrix2[1] / ((float) width));
            GLES20.glUniform4f(this.coeffsLoc, 0.299f, 0.587f, 0.114f, 0.0f);
            GLES20.glDrawArrays(5, 0, 4);
            GLES20.glViewport(0, height, uv_width, uv_height);
            GLES20.glUniform2f(this.xUnitLoc, (2.0f * transformMatrix2[0]) / ((float) width), (2.0f * transformMatrix2[1]) / ((float) width));
