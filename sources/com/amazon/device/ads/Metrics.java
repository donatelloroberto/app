...
class Metrics {
    ...
    enum MetricType {
        ...
        AD_COUNTER_IDENTIFIED_DEVICE(PushConstants.CHANNEL_ID),
        ...
        private MetricType(String aaxName2) {
            this(r2, r3, aaxName2, false);
        }

        private MetricType(String aaxName2, boolean isAdTypeSpecific2) {
            this.aaxName = aaxName2;
            this.isAdTypeSpecific = isAdTypeSpecific2;
        }
    ...
    private void sendMetrics(final WebRequest webRequest) {
        ThreadUtils.scheduleRunnable(new Runnable() {
            public void run() {
                webRequest.enableLog(Metrics.TYPED_METRIC);
                try {
                    webRequest.makeCall();
