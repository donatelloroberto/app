...
public final class zzbez {
    ...
    public final int zzacc() throws IOException {
        ...
        byte b = zzach & ByteCompanionObject.MAX_VALUE;
        byte zzach2 = zzach();
        if (zzach2 >= 0) {
            return b | (zzach2 << 7);
        }
        byte b2 = b | ((zzach2 & ByteCompanionObject.MAX_VALUE) << 7);
        byte zzach3 = zzach();
        if (zzach3 >= 0) {
            return b2 | (zzach3 << 14);
        }
        byte b3 = b2 | ((zzach3 & ByteCompanionObject.MAX_VALUE) << 14);
        byte zzach4 = zzach();
        if (zzach4 >= 0) {
            return b3 | (zzach4 << 21);
        }
        byte b4 = b3 | ((zzach4 & ByteCompanionObject.MAX_VALUE) << 21);
        byte zzach5 = zzach();
        byte b5 = b4 | (zzach5 << 28);
        if (zzach5 >= 0) {
            return b5;
        }
        for (int i = 0; i < 5; i++) {
