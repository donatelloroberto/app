...
public class FullScreenPlugin extends CordovaPlugin {
    ...
    /* access modifiers changed from: private */
    public int mLastSystemUIVisibility = 0;
    ...
    public void initialize(final CordovaInterface cordova2, CordovaWebView webView) {
        ...
        this.f5cordova.getActivity().runOnUiThread(new Runnable() {
            public void run() {
                ...
                FullScreenPlugin.this.setStatusBarBackgroundColor(FullScreenPlugin.this.preferences.getString("StatusBarBackgroundColor", "#000000"));
            }
        });
    }
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (ACTION_SET_SYSTEM_UI_VISIBILITY.equals(action)) {
    ...
    public boolean isSupported() {
        boolean supported = Build.VERSION.SDK_INT >= 14;
        this.context.sendPluginResult(new PluginResult(PluginResult.Status.OK, supported));
        return supported;
    }

    /* access modifiers changed from: protected */
    public boolean isImmersiveModeSupported() {
        boolean supported = Build.VERSION.SDK_INT >= 19;
        this.context.sendPluginResult(new PluginResult(PluginResult.Status.OK, supported));
        return supported;
    }
        ...
        this.activity.runOnUiThread(new Runnable() {
            public void run() {
                try {
                    ...
                    int unused = FullScreenPlugin.this.mLastSystemUIVisibility = 1798;
                    FullScreenPlugin.this.decorView.setOnSystemUiVisibilityChangeListener(new View.OnSystemUiVisibilityChangeListener() {
                        public void onSystemUiVisibilityChange(int visibility) {
                            if ((FullScreenPlugin.this.mLastSystemUIVisibility & 2) != 0 && (visibility & 2) == 0) {
                                FullScreenPlugin.this.resetHideTimer();
                            }
                            int unused = FullScreenPlugin.this.mLastSystemUIVisibility = visibility;
                        }
                    });
    ...
    public void resetHideTimer() {
        ...
        this.mLeanBackHandler.postDelayed(this.mEnterLeanback, 3000);
    }
    ...
    public boolean setSystemUiVisibility(final int visibility) {
        ...
        this.activity.runOnUiThread(new Runnable() {
            public void run() {
                try {
                    FullScreenPlugin.this.resetWindow();
                    FullScreenPlugin.this.decorView.setSystemUiVisibility(visibility);
                    FullScreenPlugin.this.context.success();
                } catch (Exception e) {
                    FullScreenPlugin.this.context.error(e.getMessage());
                }
    ...
    public void setStatusBarBackgroundColor(String colorPref) {
        if (Build.VERSION.SDK_INT >= 21 && colorPref != null && !colorPref.isEmpty()) {
            Window window2 = this.f5cordova.getActivity().getWindow();
            window2.clearFlags(67108864);
            window2.addFlags(Integer.MIN_VALUE);
            try {
                window2.getClass().getDeclaredMethod("setStatusBarColor", new Class[]{Integer.TYPE}).invoke(window2, new Object[]{Integer.valueOf(Color.parseColor(colorPref))});
