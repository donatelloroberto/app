...
class CollectionsKt__ReversedViewsKt extends CollectionsKt__MutableCollectionsKt {
    ...
    public static final int reverseElementIndex$CollectionsKt__ReversedViewsKt(@NotNull List<?> $receiver, int index) {
        int lastIndex = CollectionsKt.getLastIndex($receiver);
        if (index >= 0 && lastIndex >= index) {
            return CollectionsKt.getLastIndex($receiver) - index;
        }
        throw new IndexOutOfBoundsException("Element index " + index + " must be in range [" + new IntRange(0, CollectionsKt.getLastIndex($receiver)) + "].");
    }

    /* access modifiers changed from: private */
    public static final int reversePositionIndex$CollectionsKt__ReversedViewsKt(@NotNull List<?> $receiver, int index) {
